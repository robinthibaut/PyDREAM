...
exp_cond_AA = [0, 0.5, 1, 2, 4, 8, 16]
exp_cond_AG = [0, 0.5, 1, 2, 4, 8, 16]

# Experimentally measured parameter values. These will not be sampled with DREAM.
KD_AA_cat1 = np.log10(
    cox2_model.parameters["kr_AA_cat1"].value
    / cox2_model.parameters["kf_AA_cat1"].value
)
kcat_AA1 = np.log10(cox2_model.parameters["kcat_AA1"].value)
KD_AG_cat1 = np.log10(
    cox2_model.parameters["kr_AG_cat1"].value
    / cox2_model.parameters["kf_AG_cat1"].value
)
kcat_AG1 = np.log10(cox2_model.parameters["kcat_AG1"].value)
KD_AG_allo3 = np.log10(
    cox2_model.parameters["kr_AG_allo3"].value
    / cox2_model.parameters["kf_AG_allo3"].value
)
...
kf_idxs = [
    i for i, param in enumerate(cox2_model.parameters) if param.name in kfs_to_change
]
...
kd_AA_cat2 = SampledParam(
    norm,
    loc=np.log10(
        cox2_model.parameters["kr_AA_cat2"].value
        / cox2_model.parameters["kf_AA_cat2"].value
    ),
    scale=1.5,
)
kcat_AA2 = SampledParam(
    norm, loc=np.log10(cox2_model.parameters["kcat_AA2"].value), scale=0.66
)
kd_AA_cat3 = SampledParam(
    norm,
    loc=np.log10(
        cox2_model.parameters["kr_AA_cat3"].value
        / cox2_model.parameters["kf_AA_cat3"].value
    ),
    scale=1.5,
)
kcat_AA3 = SampledParam(
    norm, loc=np.log10(cox2_model.parameters["kcat_AA1"].value), scale=0.66
)
kd_AG_cat2 = SampledParam(
    norm,
    loc=np.log10(
        cox2_model.parameters["kr_AG_cat2"].value
        / cox2_model.parameters["kf_AG_cat2"].value
    ),
    scale=1.5,
)
kd_AG_cat3 = SampledParam(
    norm,
    loc=np.log10(
        cox2_model.parameters["kr_AG_cat3"].value
        / cox2_model.parameters["kf_AG_cat3"].value
    ),
    scale=1.5,
)
kcat_AG3 = SampledParam(
    norm, loc=np.log10(cox2_model.parameters["kcat_AG3"].value), scale=0.66
)
kd_AA_allo1 = SampledParam(
    norm,
    loc=np.log10(
        cox2_model.parameters["kr_AA_allo1"].value
        / cox2_model.parameters["kf_AA_allo1"].value
    ),
    scale=1,
)
kd_AA_allo2 = SampledParam(
    norm,
    loc=np.log10(
        cox2_model.parameters["kr_AA_allo2"].value
        / cox2_model.parameters["kf_AA_allo2"].value
    ),
    scale=1,
)
kd_AA_allo3 = SampledParam(
    norm,
    loc=np.log10(
        cox2_model.parameters["kr_AA_allo3"].value
        / cox2_model.parameters["kf_AA_allo3"].value
    ),
    scale=1,
)
kd_AG_allo1 = SampledParam(
    norm,
    loc=np.log10(
        cox2_model.parameters["kr_AG_allo1"].value
        / cox2_model.parameters["kf_AG_allo1"].value
    ),
    scale=1,
)
kd_AG_allo2 = SampledParam(
    norm,
    loc=np.log10(
        cox2_model.parameters["kr_AG_allo2"].value
        / cox2_model.parameters["kf_AG_allo2"].value
    ),
    scale=1,
)
...
def likelihood(parameter_vector):
    ...
    param_dict = {
        pname: pvalue
        for pname, pvalue in zip(pysb_sampled_parameter_names, parameter_vector)
    }
    ...
    box1 = (
        (1 / (10 ** KD_AA_cat1))
        * (1 / (10 ** param_dict["kr_AA_allo2"]))
        * (10 ** param_dict["kr_AA_cat3"])
        * (10 ** param_dict["kr_AA_allo1"])
    )
    box2 = (
        (1 / (10 ** param_dict["kr_AA_allo1"]))
        * (1 / (10 ** param_dict["kr_AG_cat3"]))
        * (10 ** param_dict["kr_AA_allo3"])
        * (10 ** KD_AG_cat1)
    )
    box3 = (
        (1 / (10 ** param_dict["kr_AG_allo1"]))
        * (1 / (10 ** param_dict["kr_AA_cat2"]))
        * (10 ** param_dict["kr_AG_allo2"])
        * (10 ** KD_AA_cat1)
    )
    box4 = (
        (1 / (10 ** KD_AG_cat1))
        * (1 / (10 ** KD_AG_allo3))
        * (10 ** param_dict["kr_AG_cat2"])
...
if __name__ == "__main__":
    ...
    for chain in range(len(sampled_params)):
        np.save(
            "corm_dreamzs_5chain_sampled_params_chain_"
            + str(chain)
            + "_"
        ...
        np.save(
            "corm_dreamzs_5chain_logps_chain_"
            + str(chain)
            + "_"
            + str(total_iterations),
            log_ps[chain],
    ...
    np.savetxt(
        "corm_dreamzs_5chain_GelmanRubin_iteration_" + str(total_iterations) + ".txt",
        GR,
    )
    ...
    if np.any(GR > 1.2):
        ...
        while not converged:
            ...
            print("At iteration: ", total_iterations, " GR = ", GR)
            np.savetxt(
                "corm_dreamzs_5chain_GelmanRubin_iteration_"
                + str(total_iterations)
                + ".txt",
                GR,
            )
