...
class Dream:
    def __init__(
        ...
        if self.boundaries:
            if self.total_var_dimension == 1:
                ...
                self.boundary_mask = np.ones(
                    self.total_var_dimension, dtype=bool
                )  # Mask for boundaries
            self.mins = []
            self.maxs = []
            n = 0
        ...
        # If the number of crossover values is greater than the total variable dimension, set it to be the total
        # variable dimension
        if self.nCR > self.total_var_dimension:
            self.nCR = self.total_var_dimension
            warnings.warn(
                f"The total number of crossover values specified ({nCR}) is less than the total dimension of all "
                f"variables ({self.total_var_dimension}). Setting the number of crossover values to be equal to the "
                f"total variable dimension. "
            )
        ...
        if self.total_var_dimension == 1 and adapt_crossover:
            ...
            warnings.warn(
                "The total variable dimension = 1, so crossover values will not be adapted, even though crossover "
                "adaptation was requested. "
            )
        ...
        if crossover_file:
            ...
            if self.adapt_crossover:
                warnings.warn(
                    "Crossover values loaded and adapt_crossover = True. Crossover values will be further adapted. "
                )
        ...
        if gamma_file:
            ...
            self.gamma_probabilities = [
                1 / float(self.ngamma) for i in range(self.ngamma)
            ]
        ...
        self.DEpairs = np.linspace(
            1, DEpairs, num=DEpairs, dtype=int
        )  # This is delta in original Matlab code
    ...
    def astep(self, q0, T=1.0, last_loglike=None, last_logprior=None):
        ...
        if self.iter == 0:
            ...
            try:
                with DreamParameters.nchains.get_lock():
                    self.chain_n = DreamParameters.nchains.value - 1
                    DreamParameters.nchains.value -= 1

                # Assuming the shared variables exist, seed the history with nseedchain draws from the prior
                with DreamParameters.history_seeded.get_lock() and DreamParameters.history.get_lock():
                    if not self.history_file:
                        if self.verbose:
                            print("History file not loaded.")
                        if DreamParameters.history_seeded.value == b"F":
                            if self.verbose:
                                print(
                                    "Seeding history with ",
                                    self.nseedchains,
                                    " draws from prior.",
                                )
                    ...
                    if self.verbose:
                        ...
                        print(
                            "Set probability of different crossover values to: ",
                ...
                if self.save_history:
                    with DreamParameters.history.get_lock():
                        self.len_history = len(
                            np.frombuffer(DreamParameters.history.get_obj())
                        )
        ...
        try:
            ...
            gamma_level = self.set_gamma_level(
                self.gamma_probabilities, self.gamma_level_values
            )

            with DreamParameters.history.get_lock() and DreamParameters.count.get_lock():
                # Generate proposal points
                if not run_snooker:
                    proposed_pts = self.generate_proposal_points(
                        self.multitry, q0, CR, DEpair_choice, gamma_level, snooker=False
                    )
            ...
            if self.multitry == 1:
                ...
                log_priors, log_likes = self.mt_evaluate_logps(
                    self.parallel, self.multitry, proposed_pts, self.logp, ref=False
                )
                ...
                (
                    ...
                    q_loglike_noT,
                    q_prior,
                ) = self.mt_choose_proposal_pt(log_priors, log_likes, proposed_pts, T)
                ...
                ref_log_priors, ref_log_likes = self.mt_evaluate_logps(
                    self.parallel, self.multitry, reference_pts, self.logp, ref=True
                )
            ...
            if self.multitry > 1:
                ...
                max_logp = np.amax(
                    np.concatenate((total_proposal_logp, total_reference_logp))
                )
                weight_proposed = np.exp(total_proposal_logp - max_logp)
                weight_reference = np.exp(total_reference_logp - max_logp)
                q_new = metrop_select(
                    np.nan_to_num(
                        np.log(np.sum(weight_proposed) / np.sum(weight_reference))
                    ),
                    q_proposal,
                    q0,
                )
                ...
                if run_snooker:
                    ...
                    q_new = metrop_select(
                        np.nan_to_num(total_proposed_logp - total_old_logp), q, q0
                    )
                else:
                    q_new = metrop_select(
                        np.nan_to_num(q_logp) - np.nan_to_num(self.last_logp), q, q0
                    )
                ...
                if self.multitry == 1:
                    ...
                    if self.verbose:
                        print(
                            ...
                            ref_log_ps,
                            " ratio: ",
                            np.sum(weight_proposed) / np.sum(weight_reference),
                            " at temperature: ",
                            T,
                        )
            ...
            if self.iter % self.history_thin == 0:
                with DreamParameters.history.get_lock() and DreamParameters.count.get_lock():
                    self.record_history(
                        self.nseedchains,
                        self.total_var_dimension,
                        q_new,
                        self.len_history,
                    )

            if self.iter < self.crossover_burnin + 1:
                with DreamParameters.current_positions.get_lock():
                ...
                with DreamParameters.cross_probs.get_lock() and DreamParameters.count.get_lock() and DreamParameters.ncr_updates.get_lock() and DreamParameters.current_positions.get_lock() and DreamParameters.delta_m.get_lock():
                    ...
                    if not run_snooker:
                        self.CR_probabilities = self.estimate_crossover_probabilities(
                            self.total_var_dimension, q0, q_new, CR
                        )

                    else:
                        self.CR_probabilities = self.estimate_crossover_probabilities(
                            self.total_var_dimension, q0, q_new, CR=1
                        )
            ...
            if self.iter == self.crossover_burnin:
                ...
                if self.adapt_crossover:
                    with DreamParameters.cross_probs.get_lock() and DreamParameters.count.get_lock() and DreamParameters.ncr_updates.get_lock() and DreamParameters.current_positions.get_lock() and DreamParameters.delta_m.get_lock():
                        ...
                        if not run_snooker:
                            self.CR_probabilities = (
                                self.estimate_crossover_probabilities(
                                    self.total_var_dimension, q0, q_new, CR
                                )
                            )
                        else:
    ...
    def set_current_position_arr(self, ndimensions, q_new):
        ...
        if self.nchains is None:
            current_positions = np.frombuffer(
                DreamParameters.current_positions.get_obj()
            )
        ...
        if self.iter < self.crossover_burnin + 1:
            ...
            DreamParameters.current_positions[start_cp:end_cp] = np.array(
                q_new
            ).flatten()
    ...
    def estimate_crossover_probabilities(self, ndim, q0, q_new, CR):
        ...
        DreamParameters.ncr_updates[m_loc] += 1

        current_positions = np.frombuffer(DreamParameters.current_positions.get_obj())
        ...
        DreamParameters.delta_m[m_loc] = DreamParameters.delta_m[m_loc] + change
    ...
    def estimate_gamma_level_probs(self, ndim, q0, q_new, gamma_level):
        ...
        q_new : numpy array
            new point in parameter space
        gamma_level : int or numpy array
    ...
    def set_snooker(self):
        ...
        if self.snooker != 0:
            snooker_choice = np.where(
                np.random.multinomial(1, [self.snooker, 1 - self.snooker]) == 1
            )
    ...
    def set_gamma(self, DEpairs, snooker_choice, gamma_level_choice, d_prime):
        ...
        gamma_unity_choice = np.where(
            np.random.multinomial(1, [self.p_gamma_unity, 1 - self.p_gamma_unity]) == 1
        )
    ...
    def draw_from_prior(model_vars, random_seed=False):
        ...
        for variable in model_vars:
            try:
                ...
                raise Exception(
                    "Random draw from distribution for variable %s not implemented yet."
                    % variable
                )
    ...
    def sample_from_history(nseedchains, DEpairs, ndimensions, snooker=False):
        ...
        if not snooker:
            chain_num = random.sample(
                range(int(DreamParameters.count.value + nseedchains)), DEpairs * 2
            )
        else:
            chain_num = random.sample(
                range(int(DreamParameters.count.value + nseedchains)), 1
            )
        start_locs = [int(i * ndimensions) for i in chain_num]
        end_locs = [int(i + ndimensions) for i in start_locs]
        sampled_chains = [
            DreamParameters.history[start_loc:end_loc]
    ...
    def generate_proposal_points(
        ...
        if not snooker:
            ...
            sampled_history_pts = np.array(
                [
                    self.sample_from_history(
                        self.nseedchains, DEpairs, self.total_var_dimension
                    )
            ...
            zeta = np.array(
                [
                    np.random.normal(0, self.zeta, self.total_var_dimension)
                    for i in range(n_proposed_pts)
                ]
            )
            ...
            e = np.array(
                [
                    np.random.uniform(-self.lamb, self.lamb, self.total_var_dimension)
                    for i in range(n_proposed_pts)
                ]
            )
            e = e + 1
            ...
            if n_proposed_pts > 1:
                d_prime = [
                    len(U[point][np.where(U[point] < CR)])
                    for point in range(n_proposed_pts)
                ]
                self.gamma = [
                    self.set_gamma(DEpairs, snooker, gamma_level, d_p)
                    for d_p in d_prime
                ]
            ...
            if np.any(d_prime != self.total_var_dimension):
                if n_proposed_pts > 1:
                    for point, pt_num in zip(proposed_pts, range(n_proposed_pts)):
                        proposed_pts[pt_num][np.where(U[pt_num] > CR)] = q0[
                            np.where(U[pt_num] > CR)
                        ]
            ...
            self.gamma = self.set_gamma(
                DEpairs, snooker, gamma_level, self.total_var_dimension
            )
    ...
    def snooker_update(self, n_proposed_pts, q0):
        ...
        sampled_history_pt = [
            self.sample_from_history(
                self.nseedchains, self.DEpairs, self.total_var_dimension, snooker=True
            )
            for i in range(n_proposed_pts)
        ]
        ...
        if n_proposed_pts > 1:
            D = [
                np.dot(proj_vec_diff[point], proj_vec_diff[point])
                for point in range(len(proj_vec_diff))
            ]

            # Orthogonal projection of chains_to_projected onto projection vector
            diff_chains_to_be_projected = [
                (chains_to_be_projected[point][0] - chains_to_be_projected[point][1])
                for point in range(n_proposed_pts)
            ]
            zP = np.nan_to_num(
                np.array(
            ...
            norms = [
                np.linalg.norm(proposed_pts[point] - sampled_history_pt[point])
                for point in range(n_proposed_pts)
            ]
            snooker_logp = [
                np.log(norm, where=norm != 0) * (self.total_var_dimension - 1)
                for norm in norms
            ]
    ...
    def mt_choose_proposal_pt(log_priors, log_likes, proposed_pts, T):
        ...
        best_logp_loc = int(
            np.squeeze(np.where(np.random.multinomial(1, logp_prob) == 1)[0])
        )
    ...
    def record_history(self, nseedchains, ndimensions, q_new, len_history):
        ...
        nhistoryrecs = DreamParameters.count.value + nseedchains
        start_loc = int(nhistoryrecs * ndimensions)
        end_loc = int(start_loc + ndimensions)
        DreamParameters.history[start_loc:end_loc] = np.array(q_new).flatten()

        DreamParameters.count.value += 1
        if self.save_history and len_history == (nhistoryrecs + 1) * ndimensions:
            ...
            self.save_history_to_disc(
                np.frombuffer(DreamParameters.history.get_obj()), prefix
            )
    ...
    def save_history_to_disc(self, history, prefix):
        ...
        print(
            "Saving fitted crossover values: ",
            self.CR_probabilities,
            " to file: ",
            filename,
        )
        ...
        print(
            "Saving fitted gamma level values: ",
            self.gamma_probabilities,
            " to file: ",
            filename,
        )
...
# let's first define the class NonDaemonMixin, which will stop any process
# to become a 'daemon'. Daemon are processes which are not allowed to
# create any new process and any I/O. Since the nipype workflow is
# supposed to create at some point new processes, we don't want to give
# this property to the processes.

# The following class will define the NonDaemonSpawnProcess

# if the system is on Windows or Linux, it will use a NonDaemonForkProcess

# if the system is on Linux, it will also use a NonDaemonForkServerProcess
...
class DreamPool(pool.Pool):
    def __init__(
        ...
        context_ = _nondaemon_context_mapper[context_._name]  # noqa
