    ...
    **kwargs
    ...
    if restart:
        ...
        if "model_name" not in kwargs:
            raise Exception(
                "Restart run specified but no model name to load history and crossover value files from given."
            )

    if type(parameters) is not list:
        parameters = [parameters]

    model = Model(likelihood=likelihood, sampled_parameters=parameters)  # Create model
    ...
    pool = _setup_mp_dream_pool(
        nchains, niterations, step_instance, start_pt=start, mp_context=mp_context
    )
    try:
        if tempering:

            sampled_params, log_ps = _sample_dream_pt(
                nchains, niterations, step_instance, start, pool, verbose=verbose
            )
    ...
    try:
        ...
        for iteration in range(iterations):
            if iteration % nverbose == 0:
                ...
                if verbose:
                    print(
                        "Iteration: ", iteration, " acceptance rate: ", acceptance_rate
                    )
                if iteration % 100 == 0:
                    ...
                    if verbose:
                        print(
                            "Iteration: ",
                            iteration,
                            " acceptance rate over last 100 iterations: ",
                            acceptance_rate_100win,
                        )
                    naccepts100win = 0
            old_params = q0
...
def _sample_dream_pt(nchains, niterations, step_instance, start, pool, verbose):
    ...
    if type(start) is list:
        ...
        args = list(
            zip(
                step_instances, [start] * nchains, T, [None] * nchains, [None] * nchains
            )
        )

    sampled_params = np.zeros(
        (nchains, niterations * 2, step_instance.total_var_dimension)
    )
    ...
    for iteration in range(niterations):
        ...
        if iteration % 10 == 0:
            ...
            if verbose:
                print(
                    ...
                    acceptance_rate,
                    " temp swap acceptance rate per chain: ",
                    Tacceptance_rate,
                    " and overall temp swap acceptance rate: ",
                    overall_Tacceptance_rate,
                )
            if iteration % 100 == 0:
                ...
                if verbose:
                    print(
                        "Iteration: ",
                        iteration,
                        " overall acceptance rate over last 100 iterations: ",
                        acceptance_rate_100win,
                        " temp swap acceptance rate: ",
                        Tacceptance_rate_100win,
                        " and overall temp swap acceptance rate: ",
        ...
        if np.log(np.random.uniform()) < alpha:
            if verbose:
                print(
                    "Accepted temperature swap of chains: ",
                    random_chains,
                    " at temperatures: ",
                    T1,
                    " and ",
            ...
            if verbose:
                print(
                    "Did not accept temperature swap of chains: ",
                    random_chains,
                    " at temperatures: ",
                    T1,
                    " and ",
